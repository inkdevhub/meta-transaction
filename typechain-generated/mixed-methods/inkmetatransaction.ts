...
import type * as ArgumentTypes from '../types-arguments/inkmetatransaction';
...
export default class Methods {
	...
	*/
	"getNonce" (
		...
		return queryOkJSON( this.__apiPromise, this.__nativeContract, this.__callerAddress, "getNonce", [address], __options, (result) => { return handleReturnType(result, getTypeDescription(7, 'inkmetatransaction')); });
	}
	...
	*/
	"execute" (
		...
		return txSignAndSend( this.__apiPromise, this.__nativeContract, this.__keyringPair, "execute", (events: EventRecord) => {
			return decodeEvents(events, this.__nativeContract, "inkmetatransaction");
